...
// helper function injected into site.scm if not already defined
const HELPER_RENDER_ALL_POSTS: &str = r#"
(define (render-all-posts posts)
  (map (lambda (post-metadata)
               (let ([post-id (hash-ref post-metadata 'id)]
                     [title (hash-ref post-metadata 'title)]
                     [date (hash-ref post-metadata 'date)]
                     [content (hash-ref post-metadata 'content)])
                 (list post-id
                       (post title date content post-metadata))))
       posts))
"#;
...
fn render_all_posts(
    engine: &mut Engine,
    posts_data: &[(String, SteelVal)],
) -> Result<(), Box<dyn std::error::Error>> {
    if posts_data.is_empty() {
        return Ok(());
    }

    let posts_list: SteelVal = posts_data
        .iter()
                    ...
                    if let SteelVal::ListV(pair_list) = item {
                        if pair_list.len() == 2 {
                            if let SteelVal::StringV(fname) = &pair_list[0] {
                                ...
                                let post_output_dir = format!("build/posts/{}", fname.as_str());
                                fs::create_dir_all(post_output_dir)?;

                                let post_filepath = format!("build/posts/{}/index.html", fname.as_str());
...
fn render_index(
    ...
    let index_posts_list: SteelVal = posts_data
        .iter()
        .map(|(_, alist)| alist.clone())
        .collect::<Vec<_>>()
        .into_steelval()
    ...
    match engine.call_function_by_name_with_args("index", vec![index_posts_list]) {
        Ok(index_sexp) => {
